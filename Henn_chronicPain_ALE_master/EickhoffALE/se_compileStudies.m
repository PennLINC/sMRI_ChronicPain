    ...
    for conditions = 5:size(B,2)
        ...
        if numel(B{Analysis,conditions})>0
            if B{Analysis,conditions}(1) == '+'
                useIt(Tasks(find(strcmpi({Tasks.Name},B{Analysis,conditions}(2:end)))).Experiments) = useIt(Tasks(find(strcmpi({Tasks.Name},B{Analysis,conditions}(2:end)))).Experiments) +1;
                conJ = conJ+1;
