% Anatomically labels coordinates using SPM Anatomy Toolbox
% ------------------------
% Inputs:
%
% coordList = as a string, name of .txt file of coordinates
% space = 2 if MNI; 1 else
% outputFile = as a string, name of .txt to name the anatomical labels
%   * don't put .txt at the end of this!
% tableName = the name of the csv file you'd want to save this in
%   * has to end with the extension
% ------------------------
% Outputs:
%
% text file of coordinates and what they were assigned to
% ------------------------
% Wrapper for SPM Anatomy Toolbox
% AX 03/25/2019

function anatomy_label(map,coordList, space, outputFile, tableName)
        ...
        for getPr = size(Probs,2):-1:1
            ...
            if getPr == size(Probs,2)
                ...
                if ProbMax(indxx,1)
                    ...
                    XYZtable.baLabel(indxx)={woAssign'};
                else
                    fprintf(fid,'\n');
                end
            end
...
XYZtable.Properties.VariableNames={'X','Y','Z','Macroanatomical','Cytoarchitectonic'};

writetable(XYZtable, tableName)
